...
<%
String phnid=request.getParameter("ii");
String phncname=request.getParameter("jj");
String phname=request.getParameter("kk");
String display=request.getParameter("ll");
String frontcam=request.getParameter("mm");
String rearcam=request.getParameter("nn");
String snap=request.getParameter("oo");
String price=request.getParameter("pp");
String storage=request.getParameter("qq");
String sim=request.getParameter("rr");
String color=request.getParameter("ss");
String features=request.getParameter("tt");
...
try
   {
	...
	File image= new File(myloc);
	 ...
	 PreparedStatement pst=con.prepareStatement("update mobile set phoneid=?,phoneconame=?,phonename=?,display=?,frontcam=?,rearcam=?,process=?,price=?,sim=?,storage=?,color=?,feature=?,image=? where phoneid=?");
	 ...
	 pst.setString(11,color);
	 pst.setString(12,features);
	 FileInputStream fis=new FileInputStream(image);
	 System.out.println(image);
	 pst.setBinaryStream(13, fis,fis.available());

	 pst.setString(14 ,phnid);
	 
	 int i = pst.executeUpdate();
	 if(i>0)
		 out.println("user editied");
	 else
	     out.println(i+ "not edited");
